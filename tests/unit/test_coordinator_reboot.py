"""Unit tests for BalansunCoordinator system reboot."""
...
class _RecordingSession:
    ...
    async def request(self, method: str, url: str, **kwargs: Any) -> _FakeResponse:
        ...
        if method == "POST" and url.endswith("/api/v1/system/reboot"):
            return _FakeResponse(200, {"ok": True, "message": "Reboot scheduled"})
...
def test_async_post_system_reboot_posts_system_endpoint() -> None:
    session = _RecordingSession()
    coord = _make_coordinator(session)

    async def run() -> None:
        await coord.async_post_system_reboot()

    asyncio.run(run())
    posts = [(m, u, body) for m, u, body in session.calls if m == "POST"]
    assert len(posts) == 1
    assert posts[0][1].endswith("/api/v1/system/reboot")
